...
def icv_bilinear_interpolate(r,c,img):
    ...
    if top_pixel>=0 and bottom_pixel<img.shape[0] and left_pixel>=0 and right_pixel<img.shape[1]:
        inter_top = weight_left * img[top_pixel,right_pixel] + weight_right * img[top_pixel,left_pixel]
        inter_bottom = weight_left * img[bottom_pixel,right_pixel] + weight_right * img[bottom_pixel,left_pixel]
...
def icv_get_boundaries(TM,row_max,col_max):
    bound = np.array([[0,0],[0,col_max-1],[row_max-1,0],[row_max-1,col_max-1]])
...
def icv_transformation(TM,img):
    ...
    min_row,min_col,max_row,max_col,height,width = icv_get_boundaries(TM,img.shape[0],img.shape[1])
