...
def icv_plot_hist(image_dic):
    ...
    for c in range(len(color_channel)):
        ...
        ax.plot(range(len(channel_int)), list(
            channel_int.values()), c=color_channel[c])
...
def icv_histogram(image, plot=False):
    ...
    for c in range(len(color_channel)):
        ...
        # Flatten the 2D channel into a 1D array
        channel_values = image[:, :, c].flatten()
        ...
        for key in dic.keys():
            ...
            dic[key] = dic[key] / total_pixels

        dic_list.append(dic)

        if plot:
            plt.plot(range(bin_size), list(dic.values()),
                     c=color_channel[c], label=color_channel[c])
